(function($) {
	var Defaults = {
			...
			animate: false, // 动画开关
			checkbox: false, //checkbox多选开关
			radio: false, // radio单选开关
		...
		addChildLi: function(parentNode, dataItem) {
			...
			if(this.settings.checkbox) {
				if(this.settings.parentCheck == false) {
					this.createCheckbox(operate, hasHitarea ? true : false);
				} else {
					this.createCheckbox(operate);
				}
			}
	...
	Multiple.prototype = {
		...
		findCheckbox: function() {
			var $this = this;
			this.rootree.find('.'+Classes['checkbox']);
			$.each(this.rootree.find('.'+Classes['checkbox']), function(i, cb) {
				var checkbox = $(cb);
				if($this.settings.parentCheck == false) {
		...
		createCheckbox: function(parent, disable, beforeEl) {
		...
		choose: function(checkbox) {
			...
			if(this.settings.parentCheck === true) this.filter(checkbox, status);
	...
	$.each([Multiple.prototype, Single, Ajax], function(step, OBJECT){
